...
class DatathonML:
    ...
    def __init__(self):
        ...
        XGB_parameters = {'XGB__n_estimators': [10,30,60,80,100,120,140,160,
                                                180,200,220,300,500,800]}        
    ...
    def eliminate_corr_features(self, df, threshold):
        ...
        correlation = self.calculate_corr(df) ### Computing matrix correlation
    ...
    def select_best_models(self, all_models, range_p, flag):
        ...
        if flag == 0: ### For the first approximation
            ...
            final_models = [list(best_models_sort.keys())[0], 
                            list(best_models_sort.values())[0],
                            self.best_parameters[list(best_models_sort
                                                      .keys())[0]]]
            self.best_parameters = {}
            self.results = {}
    ...
    def training_function_bayes(self, param_space, 
        ...
        self.results['LGB'] = self.compute_AUC(best_model, X_test, y_test)
        parameters = list(self.LGB_parameters.keys())
        self.best_parameters['LGB'] = [(parameters[i],
                             best_param_values[i]) 
            for i in range(len(parameters))]
        
        
    def training_function_grid(self, X_train, X_test, y_train, y_test, 
        ...
        self.best_parameters[type_clf] = grid.best_params_
    
    
    def print_final_solution(self, final_solution):
    ...
    def auto_ml(self, df):
        ...
        data_nocorr = self.eliminate_corr_features(data, 0.8)
        ...
        for name_model in best_models:
            ...
            if name_model != 'LGB':
                ...
                self.training_function_bayes(self.parameters[name_model], 
                                             X_train, 
                                             y_train, 
                                             X_test, 
                                             y_test, 75)
        
        ### Computing the final solution and printing the results
        final_solution = self.select_best_models(self.results, 0, 1)
